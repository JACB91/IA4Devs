# ✍️ Solución ejercicio Design 2 🟢

> Modulo 5: Planificacion y documentacion efectivas con IA (Parte 2)
> Curso: AI4Devs 2026/04
> Fuente: training.lidr.co

Has dado un paso importante en la planificación y gestión del desarrollo del sistema **ATS de LTI**, actuando como **Product Manager y Business Analyst**. En este ejercicio, has definido las **User Stories**, estructurado el **Backlog de producto**, probado diferentes enfoques en la generación de prompts y documentado tus conclusiones sobre su efectividad.

Ahora puedes revisar la solución oficial para comparar la estructuración y priorización del backlog, la claridad de las User Stories y el aterrizaje técnico de los tickets de trabajo. Analiza los enfoques utilizados y considera si hay ajustes que podrías realizar en tu propio planteamiento.

🔗 **Solución:** [https://github.com/LIDR-academy/AI4Devs-design-2-solved](https://github.com/LIDR-academy/AI4Devs-design-2-solved)
